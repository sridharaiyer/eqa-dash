...
def update_graphs(quarter):
    ...
    df_total_hours = get_df_total_hours(
        period_dict[quarter][0], period_dict[quarter][1])
...
################################################################################
    ...
    df_total_hours['Type'] = df_total_hours['Projects'].str.split(
        " ", n=1, expand=True)[0]
    ...
    # --------------------------------------------------------------------------------
    df_projtype = df_total_hours.groupby('Type').agg(
        [('Hours', 'sum'), ('Count', 'count')])
    df_projtype.columns = df_projtype.columns.droplevel(0)

    # --------------------------------------------------------------------------------
    # Project type hours pie
    # --------------------------------------------------------------------------------
    fig2 = px.pie(df_projtype, values='Hours',
                  names=df_projtype.index, title='Hours spent by Project Type')
    fig2.update_traces(textposition='inside',
                       textinfo='percent+label', automargin=True)
################################################################################
